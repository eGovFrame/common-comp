...
HTMLArea.prototype._insertImage = function(image) {
	...
	this._popupDialog("insert_image.jsp", function(param) {
		...
		var img = image;
		// 2013.12.18 - 이미지 추가시 alt 및 다른 태그 추가
		/*
		if (!img) {
			var sel = editor._getSelection();
			var range = editor._createRange(sel);
		...
		if (!img) {
			...
			if (HTMLArea.is_ie) { 
				...
				if (param.f_horiz && param.f_vert ) {
					...
					strNew= strNew+" style='height:"+param.f_vert+"px;width:"+param.f_horiz+"px;' ";
				} else if (param.f_horiz && !param.f_vert ) {
					strNew=strNew+" width='"+param.f_horiz+"' style='width:"+param.f_horiz+"px;' ";
				} else if (!param.f_horiz && param.f_vert ) {
