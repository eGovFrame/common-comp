...
ContextMenu.prototype.popupMenu = function(ev) {
	...
	if (!HTMLArea.is_ie) {
		...
		doc = popup.document;
		doc.open();
		doc.write("<html lang=\"ko\"><head><style type='text/css'>@import url(" + _editor_url + "plugins/ContextMenu/menu.css); html, body { padding: 0px; margin: 0px; overflow: hidden; border: 0px; }</style></head><body unselectable='yes'></body></html>");
		doc.close();
	}
