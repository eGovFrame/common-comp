...
<html lang="ko">
	<head>
		 ...
		 ******************************************************** */
		function fnRegist(){
			location.href = "/dam/map/tea/EgovComDamMapTeamRegist.do";
		}
		-->
		</script>
	</head>
	...
	<body onLoad="fn_egov_initl_MapTeam();">
		...
		<noscript class="noScriptTitle">자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다.</noscript>	
			...
			<form name="listForm" action="<c:url value='/dam/map/tea/EgovComDamMapTeamList.do'/>" method="post">
				<table width="100%" cellpadding="8" class="table-search" border="0">
		 			<tr>
		  				<td width="30%" class="title_left">
		   				<h1><img src="<c:out value="${ImgUrl}"/>icon/tit_icon.gif" width="16" height="16" hspace="3" align="middle"  alt="">&nbsp;지식맵(조직별) 목록</h1>
		   				</td>
		  				<th></th>
		  				<td width="20%">
		   					<select name="searchCondition" class="select" title="검색조건선택">
				   				...
				   				<option value='2' <c:if test="${searchVO.searchCondition == '2'}">selected="selected"</c:if>>조직ID</option>		   
			   				</select>
						</td>
		  				<td width="30%">
		  				...
		  				<th width="20%">
		   					<table border="0" cellspacing="0" cellpadding="0">
		    					<tr> 
								    ...
								    <td class="btnBackground"><a href="/dam/map/tea/EgovComDamMapTeamRegist.do"><spring:message code="button.create" /></a></td>
				...
				summary="이 표는 지식맵(조직별) 정보를 제공하며, 순번, 조직명, 조직ID, 지식URL, 분류일자 정보로 구성되어 있습니다 .">
				<caption>지식맵(조직별) 목록</caption>
					<thead>
						<tr>  
							...
							<th scope="col" class="title" width="20%" nowrap="nowrap">조직ID</th>
					...
					<tbody>
						<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
							<tr>				
								...
								<td class="lt_text3">
								<a href="/dam/map/tea/EgovComDamMapTeamDetail.do?pageIndex=${searchVO.pageIndex}&amp;orgnztId=${resultInfo.orgnztId}"><c:out value="${resultInfo.orgnztNm}"/></a>								
								</td>
								<td class="lt_text3">${resultInfo.orgnztId}</td>
