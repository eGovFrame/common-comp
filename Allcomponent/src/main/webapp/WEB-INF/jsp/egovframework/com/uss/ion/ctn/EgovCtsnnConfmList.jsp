...
<!-- ********** 여기서 부터 본문 내용 *************** -->
	...
	<table width="700" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%"class="title_left">
	   <h1><img src="/images/egovframework/com/cmm/icon/tit_icon.gif" width="16" height="16" hspace="3" style="vertical-align: middle" alt="">&nbsp;경조사승인관리 목록</h1></td>
	  <th>
	  </th>
	  <td width="50%">&nbsp;</td>
	  <th width="10%">
	       <table border="0" cellspacing="0" cellpadding="0">
	        <tr>
	          <td>&nbsp;&nbsp;</td>
	          <td><span class="button"><input type="submit" value="<spring:message code="button.inquire" />" onclick="fncSelectCtsnnConfmList(); return false;"></span></td>
	        </tr>
	       </table>
	  </th>
	 </tr>
	...
	<table width="700" border="0" cellpadding="0" cellspacing="1" class="table-register" summary="경조승인 검색조건" >
<caption>경조승인 검색조건</caption>
	  <tr>
	    ...
	    <th width="20%" height="23" class="required_text" scope="row"><label for="searchFromDate">신청일자</label><img src="/images/egovframework/com/cmm/icon/no_required.gif" width="15" height="15" alt=""></th>
	    <td width="30%" >
	...
	<table width="700" cellpadding="0" class="table-line" border="0" summary="이 표는 경조승인 관리리스트의 정보를 제공하며 순번, 경조구분, 경조명,신청자,소속,발생일자,진행구분,승인일자,승인처리로 구성되어 있습니다." >
    <caption>경조승인관리 목록</caption>
	<thead>
	<tr>
		<th class="title" scope="col" width="5%" >순번</th>
		<th class="title" scope="col" width="10%">경조구분</th>
		<th class="title" scope="col" >경조명</th>
		...
		<th class="title" scope="col" width="10%">승인처리</th>
	</tr>
	</thead>
	<tbody>
	<c:forEach items="${ctsnnManageList}" var="resultInfo" varStatus="status">
	<tr>
		<td class="lt_text3"><c:out value="${(ctsnnManageVO.pageIndex - 1) * ctsnnManageVO.pageSize + status.count}"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.ctsnnCdNm  }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.ctsnnNm  }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.usNm  }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.orgnztNm   }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.occrrDe    }"/></td>
		<td class="lt_text3">
		...
		<td class="lt_textL"><c:out value="${resultInfo.sanctnDt   }"/></td>
		<td class="lt_text3">
		<form name="item" method="post" action="<c:url value='/uss/ion/ctn/EgovCtsnnConfm.do'/>">
            ...
            <span class="button"><input type="submit"
            	  ...
            	  onclick="fncCtsnnManageConfm('<c:out value="${resultInfo.ctsnnId}"/>','<c:out value="${resultInfo.infrmlSanctnId}"/>'); return false;"></span>
        </form>
		</td>
	</tr>
	</c:forEach>
