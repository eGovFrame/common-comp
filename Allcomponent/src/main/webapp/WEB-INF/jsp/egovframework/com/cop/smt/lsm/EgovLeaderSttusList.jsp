<% 
/**
 * @Class Name : EgovLeaderSttusList.jsp
 * @Description : 간부상태관리 목록조회
...
<script type="text/javascript">
	...
	function fn_egov_select_leadersttus(pageNo) {
		...
		document.frm.action = "<c:url value='/cop/smt/lsm/mng/selectLeaderSttusList.do'/>";
		document.frm.submit();	
	}

	function fn_egov_inqire_leadersttus(leaderId) {
		document.frm.leaderId.value = leaderId;
		document.frm.action = "<c:url value='/cop/smt/lsm/mng/modifyLeaderSttus.do'/>";
		document.frm.submit();	
	}

	function fn_egov_insert_leadersttus(){	
		document.frm.action = "<c:url value='/cop/smt/lsm/mng/addLeaderSttus.do'/>";
		document.frm.submit();
	}
</script>
<title>간부상태관리 목록조회</title>
</head>
	...
	<tbody>
	 <tr>
	  <td width="40%" class="title_left">
	   <h1><img src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" style="vertical-align: middle" alt="제목아이콘이미지">&nbsp;간부상태관리 목록</h1></td>
		<td width="25%" >
	...
	<caption>간부상태 목록</caption>
	 <thead>
	  <tr>
	    ...
	    <th scope="col" class="title" width="15%" nowrap>최종수정일시</th>
	  </tr>
	 </thead>    
	 <tbody>
