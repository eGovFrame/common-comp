<%--
  Class Name : EgovAdministrationWordManageList.jsp
  Description : 행정전문용어사전관리 목록 페이지
...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olh/awm/listAdministrationWordManage.do'/>";
   	document.listForm.submit();
}
/* ********************************************************
 * 등록 처리 함수
 ******************************************************** */
function fn_egov_regist_AdministrationWordManage(){
	location.href = "<c:url value='/uss/olh/awm/registAdministrationWordManage.do' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_AdministrationWordManage(administWordId){
	var vFrom = document.listForm;
	vFrom.administWordId.value = administWordId;
 ...
 * 검색 함수
 ******************************************************** */
function fn_egov_search_AdministrationWordManage(){
	var vFrom = document.listForm;
	vFrom.choseongA.value = '';
	vFrom.choseongB.value = '';
	vFrom.cmd.value = '';
	vFrom.action = "<c:url value='/uss/olh/awm/listAdministrationWordManage.do' />";
	vFrom.submit();
...
<form name="listForm" action="<c:url value='/uss/olh/awm/listAdministrationWordManage.do'/>" method="post">
<table width="100%" cellpadding="1" class="table-search" border="0">
 <tr>
  <td class="title_left">

   <img src="<c:out value="${ImgUrl}" />tit_icon.gif" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;행정전문용어사전관리 목록</td>
  <th>
  </th>
  <td width="110px">
   ...
   <table border="0" cellspacing="0" cellpadding="0" align="center">
    <tr>
      ...
      <td><span class="button">
     	<input type="submit" value="<spring:message code="button.inquire" />" onclick="fn_egov_search_AdministrationWordManage(); return false;"></span></td>
      <td>&nbsp;</td>
      <td><span class="button">
      	<a href="<c:url value='/uss/olh/awm/registAdministrationWordManage.do' />"><spring:message code="button.create" /></a></span></td>
      <td>&nbsp;</td>
    </tr>
   </table>
  </th>
...
<tr>
    ...
    <td class="lt_text3L">
    <form name="subForm" method="post" action="<c:url value='/uss/olh/awm/detailAdministrationWordManage.do'/>">
   		<input name="administWordId" type="hidden" value="${resultInfo.administWordId}">
   		<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
   		<span class="link"><input type="submit" style="width:220px;text-align:left;" value="<c:out value="${resultInfo.administWordNm}"/>" onclick="fn_egov_detail_AdministrationWordManage('<c:out value="${resultInfo.administWordId}"/>'); return false;"></span>
    </form>
    </td>
