				...
				.jstree({
					...
					'contextmenu' : {
						'items' : function(node) {
							...
							tmp.rename.label = "이름변경";
							tmp.remove.label = "삭제";
							tmp.ccp.label = "편집";
							tmp.ccp.submenu.cut.label="이동부서선택";
							tmp.ccp.submenu.paste.label="붙여넣기";
				...
				.on('changed.jstree', function (e, data) {
					if(data && data.selected && data.selected.length) {
						...
						if(data.node.icon == "dept") {
							...
							htmlfile = "<c:url value='/html/egovframework/com/ext/ldapumt/dept_html.html' />";
						}
							
						if(data.node.icon == "user") {
							url = "<c:url value='/ext/ldapumt/dpt/getUserManage.do' />";
							htmlfile = "<c:url value='/html/egovframework/com/ext/ldapumt/user_html.html' />";
						}
