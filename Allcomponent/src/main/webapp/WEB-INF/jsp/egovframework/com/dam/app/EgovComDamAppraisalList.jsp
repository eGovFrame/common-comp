<%
 /**
  * @Class Name  : EgovComDamAppraisalList.jsp
...
<html lang="ko">
	<head>
		 ...
		 * 초기화
		 ******************************************************** */
		function fn_egov_initl_Appraisal(){
			// 첫 입력란에 포커스..
		...
		function linkPage(pageNo){
			...
			document.listForm.action = "<c:url value='/dam/app/EgovComDamAppraisalList.do'/>";
	...
	<body onLoad="fn_egov_initl_Appraisal();">
		...
		<noscript class="noScriptTitle">자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다.</noscript>	
			...
			<form name="listForm" action="<c:url value='/dam/app/EgovComDamAppraisalList.do'/>" method="post">
				<table width="100%" cellpadding="8" class="table-search" border="0">
		 			<tr>
		  				<td width="30%" class="title_left">
		   				<h1><img src="<c:out value="${ImgUrl}"/>icon/tit_icon.gif" width="16" height="16" hspace="3" align="middle"  alt="">&nbsp;지식평가관리 목록</h1>
		   				</td>
		  				<td width="20%">
				...
				summary="이 표는 지식평가관리 대상 정보를 제공하며 순번, 지식명, 지식유형, 등록자명, 평가결과, 평가일자 정보로 구성되어 있습니다 .">
				<caption>지식평가관리 목록</caption>
					<thead>
						<tr>  
							<th scope="col" class="title" width="10%" nowrap="nowrap">순번</th>
							<th scope="col" class="title" width="15%" nowrap="nowrap">지식명</th>
							<th scope="col" class="title" width="20%" nowrap="nowrap">지식유형</th>
							<th scope="col" class="title" width="20%" nowrap="nowrap">등록자명</th>
							<th scope="col" class="title" width="15%" nowrap="nowrap">평가결과</th>								
					...
					<tbody>
						<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
							<tr>
								...
								<td class="lt_text3">
								<a href="/dam/app/EgovComDamAppraisal.do?pageIndex=${searchVO.pageIndex}&amp;knoId=${resultInfo.knoId}"><c:out value="${resultInfo.knoNm}"/></a>								
								</td>
								<td class="lt_text3" nowrap="nowrap">${resultInfo.knoTypeNm}</td>								
								<td class="lt_text3" nowrap="nowrap">${resultInfo.userNm}</td>							
							    <td class="lt_text3" nowrap="nowrap">
							    <c:if test="${resultInfo.knoAps == null}">평가중</c:if>
							    <c:if test="${resultInfo.knoAps == '1'}">승인</c:if>
							    <c:if test="${resultInfo.knoAps == '2'}">반려</c:if>							    
							    </td>
							    <td class="lt_text3" nowrap="nowrap">
							    <c:if test="${resultInfo.appYmd == null}">진행중</c:if>
							    <c:if test="${resultInfo.appYmd != null}">${resultInfo.appYmd}</c:if>					    
							    </td>
							</tr>   
						</c:forEach>
