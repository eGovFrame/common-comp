<%
 /**
  * @Class Name  : EgovComDamMapMaterialList.jsp
  * @Description : EgovComDamMapMaterialList 화면
...
<html lang="ko">
	<head>
		...
		function linkPage(pageNo){
			...
			document.listForm.action = "<c:url value='/dam/map/mat/EgovComDamMapMaterialList.do'/>";
		 ...
		 ******************************************************** */
		function fnRegist(){
			location.href = "/dam/map/mat/EgovComDamMapMaterialRegist.do";
		}
		-->
		</script>
	</head>
	...
	<body onLoad="fn_egov_initl_MapMaterial();">
		...
		<noscript class="noScriptTitle">자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다.</noscript>	
			...
			<input type="hidden" name="orgnztId">
				<table width="100%" cellpadding="8" class="table-search" border="0">
		 			<tr>
		  				<td width="30%" class="title_left">
		   				<h1><img src="<c:out value="${ImgUrl}"/>icon/tit_icon.gif" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;지식맵(유형별) 목록</h1>
		   				</td>
		  				<td width="20%">
		   					<select name="searchCondition" class="select" title="검색조건선택">
				   				<option value=''>--선택하세요--</option>
				   				<option value='1' <c:if test="${searchVO.searchCondition == '1'}">selected="selected"</c:if>>조직명</option>
		  				...
		  				<th width="20%">
		   					<table border="0" cellspacing="0" cellpadding="0">
		    					<tr> 
								    ...
								    <td class="btnBackground"><a href="<c:url value='/dam/map/mat/EgovComDamMapMaterialRegist.do'/>"><spring:message code="button.create" /></a></td>
				...
				summary="이 표는 지식맵(유형별) 정보를 제공하며, 순번, 조직명, 지식유형명, 지식URL, 분류일자 정보로 구성되어 있습니다 .">
				<caption>지식맵(유형별) 목록</caption>
					<thead>
						<tr>  
							<th scope="col" class="title" width="10%" nowrap="nowrap">순번</th>
							<th scope="col" class="title" width="20%" nowrap="nowrap">조직명</th>							
							<th scope="col" class="title" width="20%" nowrap="nowrap">지식유형명</th>
							<th scope="col" class="title" width="30%" nowrap="nowrap">지식URL</th>	
							<th scope="col" class="title" width="20%" nowrap="nowrap">분류일자</th>
						</tr>
					</thead>    
					<tbody>
						<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
							<tr>
								...
								<td class="lt_text3">
								<a href="/dam/map/mat/EgovComDamMapMaterial.do?pageIndex=${searchVO.pageIndex}&amp;knoTypeCd=${resultInfo.knoTypeCd}"><c:out value="${resultInfo.knoTypeNm}"/></a>								
								</td>
								<td class="lt_text3">${resultInfo.knoUrl}</td>
								<td class="lt_text3">${resultInfo.clYmd}</td>
							</tr>   
						</c:forEach>
