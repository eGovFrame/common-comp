...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olh/omm/listOnlineManual.do'/>";
   	document.listForm.submit();
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_OnlineManual(onlineMnlId){
	var vFrom = document.listForm;
	vFrom.onlineMnlId.value = onlineMnlId;
...
function fn_egov_search_OnlineManual(){
	...
	vFrom.action = "<c:url value='/uss/olh/omm/listOnlineManual.do' />";
	vFrom.submit();
...
<!--  상단타이틀 -->
<form name="listForm" action="<c:url value='/uss/olh/omm/listOnlineManual.do'/>" method="post">
<%-- noscript 테그 --%>
...
<table width="100%" cellpadding="1" class="table-search" border="0">
 <tr>
  <td class="title_left">
   ...
   <img src="<c:out value="${ImgUrl}" />icon/tit_icon.gif" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;온라인메뉴얼관리 목록</td>
  <th>
  </th>
  <td width="110px">
  ...
  <th width="70px" align="center">
   <table border="0" cellspacing="0" cellpadding="0" align="center">
    <tr>
      ...
      <td>&nbsp;</td>
      <td><span class="button"><a href="<c:url value='/uss/olh/omm/registOnlineManual.do' />?pageIndex=<c:out value='${searchVO.pageIndex}' />"><spring:message code="button.create" /></a></span>
      </td>
      <td>&nbsp;</td>
    </tr>
   </table>
...
<input name="onlineMnlId" type="hidden" value="">
...
<tr>
    ...
    <td class="lt_text3L" nowrap>
    	<form name="subForm" method="post" action="<c:url value='/uss/olh/omm/detailOnlineManual.do'/>">
    	<input name="onlineMnlId" type="hidden" value="<c:out value="${resultInfo.onlineMnlId}"/>">
    	<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
    	<span class="link"><input type="submit" style="width:450px;text-align:left;" value="<c:out value="${resultInfo.onlineMnlNm}"/>" onclick="fn_egov_detail_OnlineManual('<c:out value="${resultInfo.onlineMnlId}"/>'); return false;"></span>
    	</form>
    </td>
