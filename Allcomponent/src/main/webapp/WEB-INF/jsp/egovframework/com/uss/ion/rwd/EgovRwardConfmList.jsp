...
<!-- ********** 여기서 부터 본문 내용 *************** -->
	...
	<table width="700" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%"class="title_left">
	   <h1><img src="/images/egovframework/com/cmm/icon/tit_icon.gif" width="16" height="16" hspace="3" style="vertical-align: middle" alt="">&nbsp;포상승인관리 목록</h1></td>
	  <th>
	  </th>
  <td width="50%">&nbsp;</td>
  <th width="10%" align="right">
	       <table border="0" cellspacing="0" cellpadding="0">
	        <tr>
	          <td>&nbsp;&nbsp;</td>
	          <td><span class="button"><input type="submit" value="<spring:message code="button.inquire" />" onclick="fncSelectRwardConfmList(); return false;"></span></td>
	        </tr>
	       </table>
	  </th>
	 </tr>
	...
	<table width="700" border="0" cellpadding="0" cellspacing="1" class="table-register"  summary="포상승인관리 검색조건" >
	  ...
	  <tr>
	    ...
	    <td width="30%" nowrap >
		 ...
		 <c:if test="${!empty rwardManageVO.searchToDate}">
	      ...
	      <input name="searchToDate" type="text" size="10" maxlength="10" value="${fromNow}" title="포상 종료일자" >
	...
	<table width="700" cellpadding="0" class="table-list" border="0" summary="포상승인관리 목록" >
	...
	<tr>
		...
		<th class="title" width="10%" scope="col">승인처리</th>
	</tr>
	</thead>
	<tbody>
	<c:forEach items="${rwardManageList}" var="resultInfo" varStatus="status">
	<tr>
		<td class="lt_text3"><c:out value="${(rwardManageVO.pageIndex - 1) * rwardManageVO.pageSize + status.count}"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.rwardCdNm    }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.rwardNm      }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.rwardDe      }"/></td>
		<td class="lt_text3"><c:out value="${resultInfo.rwardManNm   }"/></td>
		...
		<form name="item" method="post" action="<c:url value='/uss/ion/rwd/EgovRwardConfm.do'/>">
            ...
            <span class="button"><input type="submit"
                  ...
                  onclick="fncRwardManageConfm('<c:out value="${resultInfo.rwardId}"/>','<c:out value="${resultInfo.infrmlSanctnId}"/>'); return false;"></span>
        </form>
        </td>
	</tr>
	</c:forEach>
